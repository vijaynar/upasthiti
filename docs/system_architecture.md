...
*   **Coach Pricing Policies (`coach_pricing_policies`/`coach_pricing_rules`):** Policy-based student pricing engine — a coach enables any combination of Monthly Subscription, Per Class, Class Packages, Trial Session, Fine-Based (attendance-linked), and One-Time Registration, each independently configurable, plus per-student custom overrides. Distinct from `coach_financial_settings`/`coach_payouts` (academy-pays-coach payroll). See `docs/coach_pricing_policies.md`.
