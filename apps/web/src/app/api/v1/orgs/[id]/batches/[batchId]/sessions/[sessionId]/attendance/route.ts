...
import { listAttendanceEvents, recordAttendance, NotAuthorizedError, type AttendanceStatus, type AttendanceMethod } from '@abhyas/module-attendance';
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ sessionId: string }> }) {
  ...
  try {
    const event = await recordAttendance(session, { classSessionId: sessionId, enrollmentId, status, method });
