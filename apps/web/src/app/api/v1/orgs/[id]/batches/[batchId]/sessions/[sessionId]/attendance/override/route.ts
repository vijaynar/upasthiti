...
import { overrideAttendance, NotAuthorizedError, FeatureDisabledError, type AttendanceStatus } from '@abhyas/module-attendance';
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ sessionId: string }> }) {
  ...
  try {
    const event = await overrideAttendance(session, { classSessionId: sessionId, enrollmentId, status, recordedAt });
